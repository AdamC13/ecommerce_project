...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+mysqlconnector://root:A6d0a6m1!@localhost/e_commerce_project"
...
def get_product():
    query = select(Product) #Selects data from our Product Table
    result = db.session.execute(query).scalars()
    products = result.all()
...
class OrderProduct(Base):
    ...
    order_product: Mapped[int] = mapped_column(primary_key=True)
    order_id: Mapped[int] = mapped_column(db.Integer) 
...
class OrderProductSchema(ma.Schema):
    order_product = fields.Integer(required=False)
    order_id = fields.Integer(required=True)
    product_id = fields.Integer(required=True)

    class Meta:
        fields = ("order_product", "order_id", "product_id")
...
def add_order_product():
    ...
    with Session(db.engine) as session:
        new_order_product = OrderProduct(order_id=order_product_data['order_id'], product_id=order_product_data['product_id'])
...
def get_products_in_order(id):
    query = select(OrderProduct).where(OrderProduct.order_id==id)
    result = db.session.execute(query).scalars()
    order_product = result.all()

    return order_products_Schema.jsonify(order_product)
